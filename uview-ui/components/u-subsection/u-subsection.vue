<template>
	<view class="u-subsection" :style="[subsectionStyle]">
		<view class="u-item u-line-1" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'u-item-' + index]"
		 v-for="(item, index) in listInfo" :key="index" :id="`u-subsection-${index}`">
			<view :style="[textStyle(index)]" class="u-item-text u-line-1">{{ item.name }}</view>
		</view>
		<view class="u-item-bg" :style="[itemBarStyle]" style="width: 142px;"></view>
	</view>
</template>
				...
				view.exec(res => {
					...
					setTimeout(() => {
						res.map((val, index) => {
							...
							let dom = document.getElementById(`u-subsection-${index}`)
							this.listInfo[index].width = dom.clientWidth
						});
						// 初始化滑块的宽度
