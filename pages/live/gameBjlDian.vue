<template>
	<view class="live-page" id="live-page">
		...
		<view class="live-container" v-if="this.loadingState_tableDataInfo == 1" >
			...
			<!-- 视频显示区域 开始  -->
			<view class="live-box" style="" >
				<!-- 远景摄像头 开始 -->
				<view style="z-index: 13;" class="live-video" id="live-video-far" :class="{'live-video-top': videoEnlarge}" >
					<iframe class="live-details" frameborder="0" scrolling="no" :src="videoFar" ></iframe>
				</view>
				<!-- 远景摄像头 结束 -->
				<!-- 近景摄像头 开始 -->
				<view style="z-index: 13;" class="live-video" id="live-video-near" :class="{'live-video-top': !videoEnlarge}" >
					<iframe class="live-details" frameborder="0" scrolling="no" :src="videoNear" ></iframe>
				</view>
				<!-- 近景摄像头 结束 -->
				<!-- 默认加载背景 开始 -->
				<view style="z-index: 12;" class="live-loading" >
...
<!-- 			<view class="">
			...
			<!-- 露珠 开始 -->
			<view class="details_diantou" >
				<iframe class="live-details" :src="`${lzUrl}?tableId=${tableId}&user_id=${userInformation.id}&lzHeight=${lzHeight}`" ></iframe>
			</view>
			<!-- 露珠 结束 -->
		</view>
	</view>
...
<script>
	...
	export default {
		...
		　　　　});
			...
			this.lzHeight = _this.setHeight * 1 - 50 -210 - 18 // 头部距离 视频距离 统计距离 
		},
		// 页面后台运行
		onHide() {
...
<style lang="scss" scoped>
	...
	.live-page {
		...
		.live-bet-box{
			...
			height: calc(100% - 145px - 100rpx - (100vw * 0.559) - 28rpx);
			min-height: 250px;
		}
		.live-bet{
			height: 100%;
		...
		.details_diantou {
			width: 100%;
			height: calc(100% - 100rpx - (100vw * 0.559) - 36rpx);
			overflow: hidden;
			position: relative;
			aspect-ratio: 100/35;
		}
