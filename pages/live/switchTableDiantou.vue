<template>
	...
	<view class="switch-panel" id="switch-panel" :class="{'switch-run-draw': show, 'switch-close-draw':closePanelState=='close'}" @click="closeModel($event)">
		...
		<view class="switch-popup"  >
			...
			<scroll-view scroll-y="auto" style="padding-top: 10rpx;">
				<view class="tableItem" v-for="(item, index) in get_table" :key="index" @tap="intoTelTable(item)">
					<view class="tableTitle">
						<view class="title-text">
							{{item.localesTitle}}
						</view>
	...
	export default {
		...
		methods: {
			...
			intoTelTable(index) {
				...
				// 正常状态 进入台桌
				if (index.status == 1) {
					uni.setStorageSync('routerParams',{table_id: index.id, game_type_id: index.game_type})
					// 如果百家乐
					if(index.game_type == 3){
						uni.redirectTo({
						    url: '/pages/live/gameBjlDian?table_id=' + index.id + '&game_type_id=' + index.game_type
						})
					}
					// 如果龙虎
					if(index.game_type == 2){
