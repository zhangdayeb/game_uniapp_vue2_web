<template>
	<view class="san">
		<view class="san-container">
			...
			<!-- #endif -->
			<view class="san-live-box" >
				<!-- 远景摄像头 开始 -->
				<view style="z-index: 13;" class="san-live-video" id="san-live-video-far" :class="{'san-live-video-top': videoEnlarge}" >
					<iframe class="san-live-details" frameborder="0" scrolling="no" :src="videoFar" ></iframe>
				</view>
				<!-- 远景摄像头 结束 -->
				<!-- 近景摄像头 开始 -->
				<view style="z-index: 13;" class="san-live-video" id="san-live-video-near" :class="{'san-live-video-top': !videoEnlarge}" >
					<iframe class="san-live-details" frameborder="0" scrolling="no" :src="videoNear" ></iframe>
				</view>
				<!-- 近景摄像头 结束 -->
				<!-- 默认加载背景 开始 -->
				<view style="z-index: 12;" class="san-loading" >
					<view class="san-loading-panel">
						<image class="san-loading-img" src="/static/img/live/loading.svg" mode="" ></image>
						<view class="">
				...
				<!-- 倒计时圈圈 开始 -->
				<view  class="san-count-down" v-if="tableRunInfo.end_time > 0" >
					<circleProgress :percent="percent" :size="50" :circleWidth="2" :circleColor="activeColor" :innerbgColor="innerbgColor" :defaultColor="defaultColor">
						<text class="san-count-second" :class="{'san-active-color': tableRunInfo.end_time < 6}">{{tableRunInfo.end_time}}</text>
					</circleProgress>
				</view>
				<!-- 洗牌状态 标记  结束 -->
				<!-- 消息状态 通知栏  开始 -->
				<view class="san-notice" v-if="showMsgOpen">
					{{activityDescribeText}}
				</view>
				<view class="san-notice" v-if="showWinMsgOpen">
					{{showWinMsgText}}
				</view>
				<!-- 洗牌状态 标记  开始 -->
				<view class="san-shuffle" v-if="tableRunInfo.wash_status == 1" >
					{{$locales.index.shuffle}}
				</view>
				<!-- 视频放大缩小  开始 -->
				<view class="san-zoom" style="z-index: 21;">
				...
				<!-- 视频刷新按钮 -->
				<view class="san-zoom" style="z-index: 21; top:80rpx" @tap="refreshIframe()">
					<u-icon :class="{'video-animation': startAnimation}" name="reload" color="#ffffbc"  size="48" ></u-icon>
				</view>
			</view>
			<view class="san-bet-box">
				<!-- <iframe class="san-bet" frameborder="0" scrolling="no" :src="`http://192.168.1.6:8080/san?table_id=${tableId}&game_type=6&user_id=${userInformation.id}&token=${userToken}`" ></iframe> -->
				<iframe class="san-bet" frameborder="0" scrolling="no" :src="`${configService.betUrl}/san?table_id=${tableId}&game_type=${gameType}&user_id=${userInformation.id}&token=${userToken}`" ></iframe>
			</view>
			<!-- 露珠 开始 -->
			<view class="details">
				<!-- <iframe class="san-live-details" :src="`http://192.168.101.15:8848/luzhu_niu/list_three.html?table_id=${tableId}&game_type=${gameType}&user_id=${userInformation.id}`" ></iframe> -->
				<iframe class="san-live-details" :src="`${lzUrl}?table_id=${tableId}&game_type=${gameType}&user_id=${userInformation.id}`" ></iframe>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	import {THREE_URL,SocketTask} from '@/api/socket.js'
	...
	export default {
		...
		onLoad(option) {
			...
			this.lzUrl = configService.lzThreeUrlMain
		...
		onShow() {
			uni.setStorageSync('LivePageState','show')
			this.socketTask = new SocketTask(THREE_URL)
			this.initSocket()					
		...
		onReady() {
			// this.audioHandle.audioPath = 'sangong'
			uni.setStorageSync('LivePageState','show')
			setTimeout(() => {
				uni.pageScrollTo({
